...
function urlParse(url, parseQueryString, slashesDenoteHost) {
    if (url && util$2.isObject(url) && url instanceof Url)
        return url;
    var u = new Url();
...
Url.prototype.format = function () {
    ...
    if (this.query && util$2.isObject(this.query) && Object.keys(this.query).length) {
...
function createExpression(expression, propertySpec) {
    const parser = new ParsingContext(expressions$1, [], propertySpec ? getExpectedType(propertySpec) : undefined);
...
function convertPropertyFunction(parameters, propertySpec, stops) {
    ...
    if (type === 'categorical' && typeof stops[0][0] === 'boolean') {
        assert$1(parameters.stops.length > 0 && parameters.stops.length <= 2);
...
function getFunctionType(parameters, propertySpec) {
    if (parameters.type) {
        ...
        assert$1(propertySpec.expression);
...
export { Color, ParsingError, ValidationError, composite, convertFilter, derefLayers, diffStyles as diff, expression, createFilter as featureFilter, format, styleFunction as function, v8 as latest, migrate, v8, validateStyle as validate, visit };
